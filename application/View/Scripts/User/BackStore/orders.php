	               ...
	               if($contractsValidated ===1 ){
	                ...
	                <div style="display: flex; width:100%;">
			            <?php if($view->user_id == 88){
	                        ...
	                        $prxmlid = $M3Order->getMaxml($view->user_id);
							if(count($prxmlid) > 0){
						...
						?>
			            	...
			            	<div id="az5" style="width:50%">
					           	<br /><a class="btn-default sin-padding " href="<?php echo $view->url(array('controller'=>'BackStore','action'=>'exportOrders','module'=>'User'));?>" title="Exportar Ordenes">
					           	<span style="float:left;background-color:#d74710;width:8em;border-radius:50%;text-align:center;color:#fff;padding:.5em; box-shadow:-19px 12px 17px -7px rgba(13,12,12,0.58); border: 1px solid #d74710;">EXPORTACION XML AZ-5</span></a>
								<a class="btn-default sin-padding "  href="<?php echo $view->getBaseUrl('/xmlexp/exportacion'.$xmlid.'_'.$view->user_id.'.xml')?>" download="<?php echo 'exportacion'.$xmlid.'_'.$view->user_id.'.xml'; ?>" title="Descar XML">
					           	<span style="margin-left: 1.5em; float:left;background-color:#6873BC;width:8em;border-radius:50%;text-align:center;color:#fff;padding:.5em; box-shadow:-19px 12px 17px -7px rgba(13,12,12,0.58); border: 1px solid #6873BC;">DESCARGA XML AZ-5 ACTUAL</span></a>			
								<br><br><br>
				           	</div>
			            <?php } ?>
	                            ...
	                            <?php
	                                ...
	                                ?>
	                                    <tr>
	                                        ...
	                                        <?php /*if($order->getPointBuyer()&& $order->getBuyer()->getType() == DefaultDb_Entities_User::USER_CLIENT_MAS_DISTRIBUCION):?>
	                                            <td><?php echo 
	                                                ($order->getPointBuyer()->getAddress()  
	                                                    ? $order->getPointBuyer()->getFullAddress() 
	                                                    : ''
	                                                )
	                                                . ' ('. 
	                                                ($order->getPointBuyer()->getAddress() 
	                                                ?   ($order->getPointBuyer()->getAddress()->getState() 
	                                                        ? $order->getPointBuyer()->getAddress()->getState()->getName() : ''
	                                                    )
	                                                :''
	                                                )
	                                                . ')' ;?></td>
	                                        <?php //else:?>
	                                            <td><?php //echo $order->getBuyer()->getCommercialName();?></td>
	                                        <?php endif;*/?>
	                                        <td><?php
	                                        if($order->getBuyer()->getId() != 24){
												echo (strlen($order->getBuyer()->getCommercialName()) == 0)?$order->getBuyer()->getFirstName()." ".$order->getBuyer()->getLastName():$order->getBuyer()->getCommercialName();
											}else{
												if($order->getPointBuyer()){
	                                        ...
	                                        <!-- td><?php //echo $view->Orders()->printComments($order->getId());?></td--><!--funcion generada en el helper para consultar los comentarios sobre la orden-->
	                                        ...
	                                        <td><?php 
	                                        	...
	                                        	if($view->Orders()->arriveDate($order,'1') != ""){
	                                        		echo $view->Orders()->arriveDate($order,'1');
	                                        		...
	                                        		echo '<br/><a class=" btn-default sin-padding" title="Actualizar Fecha" href="'.$view->url(array('controller' => 'BackStore', 'action' => 'RefreshDel', 'param' => $order->getId()), true).'" ><span class="pers-btn icono-refrescar-fila icono-tam-tabla"></span></a>';
	                                        	}else{
													echo "No programado";
												}
	                                        ?></td>
	                                        <td><?php 
												 if($view->Orders()->arriveDate($order,'2') != ""){
	                                        	 	echo $view->Orders()->arriveDate($order,'2');
												 	//echo '<a class=" btn-default sin-padding" href="'.$view->url(array('controller' => 'BackStore', 'action' => 'getVolR', 'param' => $order->getId()), true).'" ><span class="pers-btn icono-refrescar-fila icono-tam-tabla"></span></a>';
												 	echo '<br/><a class=" btn-default sin-padding" title="Actualizar Fecha" href="'.$view->url(array('controller' => 'BackStore', 'action' => 'RefreshArr', 'param' => $order->getId()), true).'" ><span class="pers-btn icono-refrescar-fila icono-tam-tabla"></span></a>';
	                                        	}else{
													echo "No programado";
												}
											?></td>
	                                        <td><?php
	                                        		echo '<a class="btn btn-default btn18px" title="Ver Detalles" href="'.$view->url(array('controller' => 'BackStore', 'action' => 'viewOrder', 'id' => $order->getId()),true).'"><span class="pers-btn icono-ver-actividad icono-tam-tabla"></span></a><br />';
	                                        		//echo '<a href="'.$view->url(array('controller' => 'BackStore', 'action' => 'viewOrder', 'id' => $order->getId())).'">Orden '.$order->getId().'</a>';
	                                        ...
	                                        <?php 
	                                        	...
	                                        	if($order->getShippingStatus() == 0){
													...
													echo '<a class="allbuttons buttonin" style="float:left;background-color:#0f110f;" href="' . $view->url(array('module' => 'User', 'controller' => 'Dompdf', 'action' => 'index', 'id' => $order->getId())) . '" ><img src="'.$view->getBaseUrl('/images/iconos/documentpdfwhite.png').'" style="margin-right:0.5em;" />Generar etiqueta</a>';
	                        ...
	                        <tbody>
	                            <?php
	                                ...
	                                ?>
	                                    <tr>
	                                        <td><?php echo $order->getId(); ?></td>
	                                        <td><?php echo $order->getBuyer()->getFullName(); ?></td>
	                                        <td><?php echo $order->getPaymentStatusString(); ?></td>
	                                        <td><?php echo $order->getShippingStatusString(); ?></td>
	                                        <td><?php echo $order->getCreationDate() != null ? $order->getCreationDate()->format('Y-m-d') : ""; ?></td>
	                                        <td><?php echo $active = ($order->getOrderStatus() == DefaultDb_Entities_M3CommerceOrder::ORDER_STATUS_ACTIVE) ? 'Activa' : 'Inactiva' ;?></td>
	                                        <td><?php echo '<a href="'.$view->url(array('controller' => 'BackStore', 'action' => 'viewOrder', 'id' => $order->getId())).'">Orden '.$order->getId().'</a>';?></td>
	                                    </tr>
	                                <?php
	                                    }
	                                }
	                ...
	                {
	                    if($totalBranches > 0 && $branchesValidated>0)
	                        echo '<div class="alert alert-error">Actualmente no tiene entregas.</div>';
	                }
                }else{
                ?>
                	<div class="alert">
