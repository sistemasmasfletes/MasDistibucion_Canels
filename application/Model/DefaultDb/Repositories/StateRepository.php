...
class DefaultDb_Repositories_StateRepository extends EntityRepository{
    ...
    public function getEstados($page, $rowsPerPage, $sortField, $sortDir, $id, $estadoName, $paisName, $abbreviation, $userId) {
        ...
        $selectFields = " s.id id, s.name name, s.abbreviation abbreviation, p.id countryId, p.chrNombre paisName, s.chrEstatus ";
    ...
    public function getEstadoById($id) {
        ...
        $sqlSelect = "  s.id id, s.name name, s.abbreviation abbreviation, s.country_id countryId, s.chrEstatus ";
        $query = "SELECT [FIELDS] "
                . " FROM states s "
                . " WHERE s.id = :id";
    ...
    public function saveEstado($id, $abbreviation, $countryId, $name,$chrEstatus){
        $em = $this->getEntityManager();
        ...
        $nuevoEstado->setEstatus(($chrEstatus)? 1 : 0);
