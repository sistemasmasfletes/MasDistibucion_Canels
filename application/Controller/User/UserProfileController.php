...
class User_UserProfileController extends Model3_Controller
{
    ...
    public function fiscaldatAction()
    {
        ...
        if ($this->getRequest()->isPost())
        {
            ...
            $city = $em->find('DefaultDb_Entities_City', $post['city']);
            $user = $em->find('DefaultDb_Entities_User', $credentials['id']);
            $user->setCommercialName($post['commercialName']);			$user->setBusinessName($post['businessName']);
            //$user->setDayInvoice($post['dateInvoice']);
            $user->setState($state);
            $user->setCountry($country);
            $user->setCity($city);
			...
			$user->setBank($post['bank']);
			$user->setAccount($post['account']);
			$user->setClabe($post['clabe']);
			$user->setMail($post['email']);
			$user->setPhone($post['phone']);
			$user->setDelyPay((isset($post['dely_pay']))?$post['dely_pay']:"0");
			$user->setCrdPay((isset($post['crd_pay']))?$post['crd_pay']:"0");
