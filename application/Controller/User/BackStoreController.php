...
class User_BackStoreController extends JController
{
    ...
    public function aordersAction(){
        ...
        try {
            ...
            $ordersRepo = $em->getRepository('DefaultDb_Entities_M3CommerceOrder');
            $orders = $ordersRepo->getOrdersListDQL($page, $rowsPerPage, $sortField, $sortDir, $sidx, $id, $Fecha, $Hora, $Nombre, $Formato, $HoraActual, $Codigo, $Vehiculo, $Estado);
            
            $arrayorders = array();
            
            foreach ($orders['data'] as $datos){
            	
            	$salida = $ordersRepo->getTimesDQL($datos['id'],'1');
            	$llegada = $ordersRepo->getTimesDQL($datos['id'],'2');
            	
            	$arrayorders[] = array(
            		"id" => $datos['id'],
            		"Fechapedido" => $datos['Fechapedido'],
            		"commercial_name" =>$datos['commercial_name'],
            		"exit" => $salida,
            		"arrived" => $llegada
            	); 
            }
            
            $result = array('data'=>$arrayorders,'meta'=>array($orders['meta']));
...
/*        $user = $em->find('DefaultDb_Entities_User', $sellerId);
        ...
        $orderep = $em->getRepository('DefaultDb_Entities_M3CommerceOrder');
        $orders = $orderep->findBy(array('seller' => $sellerId),array('id' => 'DESC'),400);
        ...
        $this->view->user_id = $sellerId; 
        /*var_dump($this->view->exmlexp);
        if($this->view->exmlexp){
        	echo $this->view->exmlexp;
        }*/
    ...
    public function viewOrderAction()
    {
        ...
        $this->view->getJsManager()->addJsVar('urladdprducts', '\'' . $this->view->url(array('action' => 'addProductstoOrder')) . '\'');
        ...
        if($order->getBuyer()->getId() == 24){// esto se cumple por que si es cliente comodin 24 le orden tiene incluido el id de punto de entrega desde el principio
        	$arrquery = array('client' => $order->getBuyer()->getId(), 'point' => $order->getPointBuyer()->getId());
        }else{//en este caso aun no se indica el punto de entrega pero si a un cliente especifico
        	$arrquery = array('client' => $order->getBuyer()->getId());
        } 
        
        $branchesUserRepos = $em->getRepository('DefaultDb_Entities_BranchesUser');
        $branches = $branchesUserRepos->findBy(array('client' => $sellerId));
        $branchesclient = $branchesUserRepos->findBy($arrquery);
        ...
        $this->view->branchesclient =$branchesclient;
    ...
    public function viewOrderShoppingAction()
    {
        ...
        $branchesUserRepos = $em->getRepository('DefaultDb_Entities_BranchesUser');
        $branches = $branchesUserRepos->findBy(array('client' => $sellerId));
        $branchesseller = $branchesUserRepos->findBy(array('client' => $order->getSeller()->getId()));
...
//        $this->view->useTwitterWidgets = true;
    ...
    public function creteShippingAction()
    {
        ...
        if($order->getBuyer()->getId() == $user->getId()){
        	$this->view->payments = $em->getRepository('DefaultDb_Entities_Pagos')->findOneBy(array("compraVenta" => $orderId,"usuario" => $user->getId()));
        }else{
        	$this->view->payments = NULL;
        }
        
       	//$this->view->payments = ($payments) ? $payments : NULL;//////AGREGANDO EL METODO DE PAGOS 
        ...
        if($this->getRequest()->isPost() && $order instanceof DefaultDb_Entities_M3CommerceOrder)
        {
            ...
            try{
                ...
                foreach($post['unity'] as $key => $value)
                {
                    ...
                    {                     
                        ...
                        if($agregados<=0){
                            ...
                            $order->setContactR($post['contactres']);
                            $order->setContactS($post['contactsend']);
                            $order->setContent($post['contentdesc']);
                            $order->setProgramer($credentials['id']);                            
                            ...
                            foreach ($sequentialActivities as $sa) {
                                $arraySequentialActivities[]=array( "routePointId"=>$sa->getRoutePoint()->getId()
                                									...
                                									,"routeDate"=>$sa->getRouteDate()//EAAL
                                                                    );                           
                            }                           
                        ...
                        {                             
                            ...
                            $newOrderByPackage->setContactR($post['contactres']);                            $newOrderByPackage->setContactS($post['contactsend']);                            $newOrderByPackage->setContent($post['contentdesc']);                            $newOrderByPackage->setProgramer($credentials['id']);                            
                ...
                foreach ($arrayNewOrders as $regOrden) {
                    ...
                    $arrayActivityPoint = $routePointActivitiRepo->createRoutePointActivites($rpActivities,1);//se genera el ruteo 
                    
                    foreach ($rpActivities as $rpadAc) {/********************************AGREGAR ACTIVADES SECUENCIALES A ORDENES ADICIONALES EAAL*******************************************/
                    	$aditinalseq = $routePointActivitiRepo->aditionalSeq($rpadAc);
                    }
            	...
            	$correo = "<html><body>
	            		<span>Hola ".$order->getBuyer()->getFirstName()." ".$order->getBuyer()->getLastName().",
	            		<br />
	            	 	revisa tus pedidos en masdistribucion.com,
	            		<br >
	            		el proveedor: ".$order->getSeller()->getFirstName()." ".$order->getSeller()->getLastName()."
	            		programo el envio del paquete con  la orden: ".$order->getId()."<br />
            	...
            	$data = array(
            			'title' => 'Nuevo envio',
            			'body' => $order->getSeller()->getBusinessName().' programo el envio de tu paquete '.$order->getId().', revisa la sección de recolecciones.',
            	);
            	 
            	if($order->getBuyer()->getToken() != ""){
            		$this->sendPushNotification($order->getBuyer()->getToken(),  $data);            	
            	}
				/***********************NOTIFICACIONES POR MAIL Y APP A USUARIOS EAAL*******************************************/
        ...
        $conversion = $em->getRepository('DefaultDb_Entities_Conversion')->findOneBy(array('moneda' => $cliente->getMoneda()->getId()));
        ...
        $this->view->ClienteConversion = number_format(number_format($cliente->getCredito(),2, '.', '')/$conversion->getCreditos(),2);
        ...
        $this->view->getJsManager()->addJsVar('sellerId',$order->getSeller()->getId());
        $this->view->getJsManager()->addJsVar('buyerId',$order->getBuyer()->getId());
    ...
    public function createOrderAction()
    {
        ...
        $query = $em->createQuery("SELECT DISTINCT b.id,b.commercialName  FROM DefaultDb_Entities_FavoriteUsers a JOIN  a.favoriteClient b  WHERE a.client=:clientid and a.type=1 ");
        $query->setParameter("clientid", $user);
        $favoriteUsers=$query->getResult();
        ...
        if($this->getRequest()->isPost())
        {
            ...
            { 
            	...
            	$pointSeller = $branchesRepository->createQueryBuilder('t')///CORRECCION PARA ASIGNAR LA ORDEN AL USUARIO SI EL PUNTO LO TIENE EAAL(09-07-19) 
            	//->where("t.point = :id")
            	->where("t.id = :id")
            	->setMaxResults(1)
            	->orderBy('t.client', 'DESC')
            	->setParameter('id',$post['hiddenBranch'])
            	->getQuery()->getSingleResult();
            	
            	//$client_id = $pointSeller[0]->getClient()->getId();
            	$client_id = $pointSeller->getClient()->getId();
                if($client_id === 24 && $isBuyer){//si se cumple entonces se esta solicitando una recoleccion aun punto sin usuario por lo  que se define aqui mismo el punto de recoleccion
                	$order->setPointSeller($pointSeller->getPoint());
                }else{
...
//                $order->setPointBuyer($pointBuyer->getPoint());
    ...
    public function CancelOrdSAction(){/////////////////////CANCELACION DE OREDENES NO PROGRAMADAS//////////////////////////////////////////////
    	...
    	$from = $this->getRequest()->getParams()['frm'];
    	...
    	$Fus = $em->getRepository('DefaultDb_Entities_FavoriteUsers')->findOneBy(array('order' => $post));
    	$Pks = $em->getRepository('DefaultDb_Entities_PackageToOrder')->findOneBy(array('order' => $post));
    	$client = $em->getRepository('DefaultDb_Entities_User')->findOneBy(array('id' => $M3Order->getBuyer()));
    	...
    	if(sizeof($Pgs) > 0){
    		foreach ($Pgs as $d) {
    			...
    			foreach ($BlPgs as $e) {
    				...
    				if ((int)$e->getEstatus() == 2 && $e->getCliente()->getId() == $client->getId()) {//Si el monto fue pagado se suma a la variable para el reembolos de los creditos
    					$tcreditos = $tcreditos + (float) $e->getMonto();
    				}
    				$em->remove($e);
    			}
    	...
    	$client->setCredito($client->getCredito() + $tcreditos); //Se devuelven los creditos referentes a los pagos congelados hechos a la orden cancelada
    	$em->persist($client);
    	$em->flush();

    	//var_dump($tcreditos);
    	 
    	$this->redirect('User/BackStore/'.$from);
    ...
    public function CancelOrdCAction(){
		$this->view->setUseTemplate(false);
	    $post = (int)$this->getRequest()->getParams()['param'];
	    $from = (int)$this->getRequest()->getParams()['frm'];
	...
	public function exportOrdersAction(){/////////////////////EXPORTACION DE OPERACIONES A MF//////////////////////////////////////////////
		...
		$credentials = Model3_Auth::getCredentials();
		$user = $em->find('DefaultDb_Entities_User', $credentials['id']);
		$conversion = $em->getRepository('DefaultDb_Entities_Conversion')->findOneBy(array('moneda' => $user->getMoneda()->getId()));
		
		if($user->getRfc() != ""){
			
			$M3Order = $em->getRepository('DefaultDb_Entities_M3CommerceOrder');
			//$dnow = new DateTime("now");
			 
			$M3Order3 = $M3Order->createQueryBuilder('M')//LISTA DE LAS ORDENES A GENERAR DE TODOS LOS CLIENTES CON O SIN USUARIO
			->leftJoin('M.pointBuyer','p')
			->leftJoin('M.buyer','cliente')
			->leftJoin('M.products','producto')
			->leftJoin('M.packages','paquete')
			->Where('M.seller = '.$user->getId())
			->andWhere("M.stexp is NULL")
			//->andWhere("M.shippingStatus = 3")//SOLO ORENEDES ENTREGADAS
			->andWhere("M.shippingStatus <> 0")//TODAS LAS ORDENES EXCEPTO LAS QUE NO ESTAN PROGRAMADAS
			->andWhere('M.pointSeller is not NULL')
			//->andWhere('producto.product is not null')
			//->andWhere('M.buyer <> 24')//CON ESTO SE FORZA A SOLO CLIENTES CON USUARIO hasta no resolver lo del nombre de empresa y razon social no descomentar esto
			->andWhere('M.id > 405')///PARA PRUEBAS CON GOSOZA
			//->andWhere('M.buyer in(89,106)')///PARA PRUEBAS CON GOSOZA Y PARROQUIA
			//->groupBy('M.buyer')
			->orderBy('M.buyer', 'ASC')
			->getQuery();
			
			foreach ($M3Order3->getResult() as $r){
				//echo $r->getId()."<br />";
					
				if($r->getBuyer()->getId() == 24){// SE VUELVE A GENREAR EL RFC PROVICIONAL DE LA EMPRESA SIN USUARIO EL CUAL DEBERA SER IGUAL AL QUE SE GENERO YA ANTERIORMENTE
					
					/***********************************************************************************/
					/////////VERIFICACION DE QUE EL PUNTO 
					/////////HA SIDO ASIGNADO 
					/////////O NO AUN USUARIO
					/***********************************************************************************/
					//$branches = $em->getRepository('DefaultDb_Entities_BranchesUser')->findBy(array('point' =>$r->getPointBuyer()->getId()));
					$branches = $em->getRepository('DefaultDb_Entities_BranchesUser');
					
					$branch = $branches->createQueryBuilder('M')
					->Where("M.point = ".$r->getPointBuyer()->getId())
					->andWhere('M.client <> 24')
					->orderBy('M.client', 'ASC')
					->getQuery();
						
					/*echo count($branch->getResult())."<br>";
					foreach ($branch->getResult() as $br){
						echo "orden". $r->getId()." punto: ".$r->getPointBuyer()->getId()/*." usuario: ".$br->getClient()->getId()."<br>";
					}*/
										
					if(count($branch->getResult()) > 0){//SI ESTO SE CUMPLE DEBERIA DE REASIGNAR EL RFC A LA OPERACION
						foreach ($branch->getResult() as $br){
							if($br->getClient()->getRfc() == ""){//SI SE CUMPLE ESTA CONDICION QUIERE DECIR QUE LA EMPRESA TIENE USUARIO PERO NO SE HAN CAPTURADO TODOS SUS DATOS CORRECTAMENTE
								$rfc = 0;
								//$rfc = "no se exporta";
							}else{//EN ESTE CASO SE USA EL RFC CAPTURADO POR EL USUARIO ASIGNADO A LA EMPRESA
								$rfc = $br->getClient()->getRfc();
							}	
						}
						//echo $r->getPointBuyer()->getId()."<br />";
					}else{//SI NO SE CUMPLE DEBE GENERAR EL RFC PROVICIONAL
						//echo "aqqui no es objeto";
						$rfc = mb_strtoupper(
								substr($r->getPointBuyer()->getName(),0,2).
								str_replace("-","",$r->getPointBuyer()->getCode()).
								'A'.$r->getBuyer()->getId()
						);
					}
					
					//echo "orden: ".$r->getId()." -- ".$r->getPointBuyer()->getId()." rfc asigando: ".$rfc."<br>";
					
				}else{// SI ENTRA AQUI SE BUSCARA EL RFC DE LA EMPRESA
					if($r->getBuyer()->getRfc() == ""){//SI SE CUMPLE ESTA CONDICION QUIERE DECIR QUE LA EMPRESA TIENE USUARIO PERO NO SE HAN CAPTURADO TODOS SUS DATOS CORRECTAMENTE
						$rfc = 0;
						//$rfc = "no se exporta";
					}else{//EN ESTE CASO SE USA EL RFC CAPTURADO POR EL USUARIO ASIGNADO A LA EMPRESA
						$rfc = $r->getBuyer()->getRfc();
					}
				}
			
				//echo $rfc."<br >";
				
				if(!is_numeric($rfc)){// SE VERIFICA SI LA EMPRESA INDICADA EN LA OPERACION TIENE RFC ESTABLECIDO POR EL USUARIO O GENERADO AUTOMATICAMENTE DE NO SER ASI LA OPERACION SE EXCLUIRA COMPLETAMENTE
					$rpr = "";
					$metaid = "";
					///EL METADATO SE DEBE GENERAR EN BASE AL PAQUETE O AL PRODUCTO UNIENDO DOS CONCEPTOS DE MD EN UNO DE MF/////
			
					foreach($r->getProducts() as $producto){// SE OBTIENEN LOS DATOS DEL PRODUCTO /////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						$rpr =  $producto->getProduct()->getName();									/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						$quantity = $producto->getQuantity();										/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						$metaid = $producto->getProduct()->getId();									/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						//$price = $producto->getProduct()->getPrice();								/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						$price = 0;
						//echo $rfc." - ".$rpr." -- ".$metaid."<br />";								/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
					}
						
					if($metaid != ""){//SI SE CUMPLE SE TRATA DE UN PRODUCTO EL CUAL PUEDE SER FACTURADO	/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
						$metaclave = "PR".$r->getBuyer()->getId().$metaid; //DATO PARA REFERENCIA			/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
					}else{// SI NO SE CUMPLE SE TRATA DE UN PAQUETE											/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
							
						foreach ($r->getPackages() as $paquete){
							if(is_object($paquete->getPackage())){
							$metaid = $paquete->getPackage()->getId();
							//$quantity = $paquete->getNumPackage();
							$quantity = 0;
							$rpr = $paquete->getPackage()->getDescription();
							//$price = $paquete->getPrice()/$conversion->getCreditos();
							$price = 0;
							}else{
								echo "No se genero en esta operacion".$r->getId()."<br />";
							}
						}
							
						//$metaid = $r->getPackages();
						$metaclave = "PK".$r->getBuyer()->getId().$metaid; //DATO PARA REFERENCIA
					}		/////COMENTADO PARA EXCLUIR LA INFORMACION DE LOS PRODUCTOS Y SOLO USAR LOS PAQUETES DE LAS ORDENES
			
					$aord[] = array(
							'idop' => $r->getId(),
							'metaclave' => $metaclave,
							'name' => $r->getBuyer()->getCommercialName(),
							//utf8_decode(mb_strtoupper($r->getBuyer()->getCommercialName(),'utf-8'))
							'rfc' => $rfc,
							'idusrMD' => $r->getBuyer()->getId(),
							'exp' => $r->getExp(),
							'producto' => $rpr,
							'quantity' => $quantity,
							'price' => number_format($price,2, '.', ''),
							'businessname' => $r->getBuyer()->getBusinessName()
			...
			if(isset($aord)){
				...
				$prxmlid = $M3Order->getMaxml($user->getId());
				$xmlid = 1;
				if(count($prxmlid) > 0){
					foreach($prxmlid as $r){
						$xmlid = $r['folio'] + 1;
					}
				}
				
				$M3Order1 = $M3Order->createQueryBuilder('M')//LISTAS DE EMPRESAS NUEVAS SIN USUARIO EN MASDISTRIBUCION SE GENERA UN RFC PROVICIONAL
				->leftJoin('M.pointBuyer','p')
				->Where('M.seller ='.$credentials['id']) 
				->andWhere('M.pointSeller is not NULL')
				->andWhere('M.buyer = 24')
				//->andWhere('M.buyer in(89,106)')///PARA PRUEBAS CON GOSOZA Y PARROQUIA
				->groupBy('M.pointBuyer')
				->orderBy('M.buyer', 'ASC')
				->getQuery();
				$a1 = array();
				foreach ($M3Order1->getResult() as $r){
					$a1[] = array('name' => $r->getPointBuyer()->getName(),
								'businessname' => $r->getBuyer()->getBusinessName(),
								'rfc' => mb_strtoupper(
										substr($r->getPointBuyer()->getName(),0,2).
										str_replace("-","",$r->getPointBuyer()->getCode()).
				...
				$M3Order2 = $M3Order->createQueryBuilder('M')//LISTA DE EMPRESAS NUEVAS CON USUARIO EN MASDISTRIBUCION NO SE GENERA RFC PERO EL USUARIO DEBE REGISTRARLO O NO SE EXPORTARA
				->leftJoin('M.pointBuyer','p')
				->leftJoin('M.buyer','cliente')
				->Where('M.seller = '.$credentials['id'])
				->andWhere('M.pointSeller is not NULL')
				...
				->getQuery();
				$a2 = array();
				foreach ($M3Order2->getResult() as $r){
					$a2[] = array(
								'rfc' => $r->getBuyer()->getRfc(),
								'name' => $r->getBuyer()->getCommercialName(),
								'businessname' => $r->getBuyer()->getBusinessName(),
				...
				$aclients = array_merge($a1,$a2);
				
				$dato = "";
				$dato1 = "";
				sort($aord);
				foreach($aord as $item){//generacion del arreglo con las claves de los articulos a generar
					if($dato != $item['metaclave'] || $dato1 != $item['rfc']){
						$arrayarticles[] = array(
											'metaclave' => $item['metaclave'],
											'rfc' => $item['rfc'],
											'price' => $item['price'],
				...
				$usuario->SetAttribute("razonsocial",$user->getCommercialName());
				$usuario->SetAttribute("usuario",$user->getUsername());
				$usuario->SetAttribute("rfc",$user->getRfc());
				$usuario->SetAttribute("id",$user->getId());
				$usuario->SetAttribute("idxml",$xmlid);
				$usuario = $xml->appendChild($usuario);
				
				$clientes = $xml->createElement('Clientes');
				$clientes = $usuario->appendChild($clientes);
				
				sort($aclients);
				foreach($aclients as $item){
					$cliente = $xml->createElement('Cliente');
					$cliente->SetAttribute("idMD",$item['idusrMD']);
					$cliente->SetAttribute("rfc",$item['rfc']);
					$cliente->SetAttribute("nombre",mb_strtoupper($item['name'],'utf-8'));
					$cliente->SetAttribute("razonsocial",mb_strtoupper($item['businessname'],'utf-8'));
					$cliente = $clientes->appendChild($cliente);
				}
				
				$articulos = $xml->createElement('Articulos');
				$articulos = $usuario->appendChild($articulos);
				
				foreach($arrayarticles as $item){
					$articulo = $xml->createElement('Articulo');
					//$articulo->SetAttribute("idop",$item['idop']);
					$articulo->SetAttribute("rfc",$item['rfc']);
					$articulo->SetAttribute("producto",$item['producto']);
					$articulo->SetAttribute("metaclave",$item['metaclave']);
					//$concepto->SetAttribute("nombre",mb_strtoupper($item['name'],'utf-8'));
					$articulo->setAttribute("precio",$item['price']);
					$articulo = $articulos->appendChild($articulo);
				}
				
				$conceptos = $xml->createElement('Ordenes');
				$conceptos = $usuario->appendChild($conceptos);
				
				sort($aord);
				foreach($aord as $item){
					$concepto = $xml->createElement('Orden');
					$concepto->SetAttribute("idop",$item['idop']);
					$concepto->SetAttribute("rfc",$item['rfc']);
					$concepto->SetAttribute("producto",$item['producto']);
					$concepto->SetAttribute("meta",$item['metaclave']);
					$concepto->SetAttribute("idusrMD",$item['idusrMD']);
					$concepto->SetAttribute("quantity",$item['quantity']);
					$concepto->setAttribute("precio",$item['price']);
					$concepto->setAttribute("total",$item['price']*$item['quantity']);
					//$concepto->SetAttribute("c",$item['c']);
					//$concepto->SetAttribute("nombre",mb_strtoupper($item['name'],'utf-8'));
					$concepto->SetAttribute("razonsoc",mb_strtoupper($item['businessname'],'utf-8'));
				...
				if($xml->save('xmlexp/exportacion'.$xmlid.'_'.$user->getId().'.xml')){
					$M3Order->newXml($user->getId(),$xmlid,$this->view->getBaseUrlPublic().'xmlexp/exportacion'.$xmlid.'_'.$user->getId().'.xml');
					
					foreach($aord as $item){
						$r = $M3Order->findOneBy(array('id' => $item['idop']));
						$r->setExp('1');
						$em->persist($r);
						$em->flush();
					}
					
					/*$file_example = 'xmlexp/exportacion'.$xmlid.'_'.$user->getId().'.xml';
				...
				echo "No existe nueva informacion para exportacion!!!!!<br />";
				echo '<a href="'.$this->view->url(array('controller' => 'BackStore','action' => 'orders')).'"><span style="float:left;background-color:green;width:8em;border-radius:5px;text-align:center;color:#fff;padding:.5em;">REGRESAR</span></a>';
				//$this->redirect('User/BackStore/orders');
			}
		}else{
			echo "Tienes que capturar todos tus datos fiscales para acceder a la importacion!!!!<br />";
			echo '<a href="'.$this->view->url(array('controller' => 'BackStore','action' => 'orders')).'"><span style="float:left;background-color:green;width:8em;border-radius:5px;text-align:center;color:#fff;padding:.5em;">REGRESAR</span></a>';
		}
	}
	...
	public function addProductstoOrderAction(){
		...
		$order = $catalogs = $em->getRepository('DefaultDb_Entities_M3CommerceOrder')->findOneBy(array('id' => $post['orderid']));
		$res = "TRUE";
		
		foreach ($post['arrayp'] as $item){

			$product = $catalogs = $em->getRepository('DefaultDb_Entities_Product')->findOneBy(array('id' => $item['id']));
			$newproduct = new DefaultDb_Entities_M3CommerceProductToOrder();
				
			$newproduct->setOrder($order);
			$newproduct->setProduct($product);
			$newproduct->setQuantity($item['quantity']);
			$newproduct->setPrice($product->getPrice()*$item['quantity']);
