function CausesIndexController($scope,$timeout,$state,$stateParams,PATH,PARTIALPATH,ModalService,JQGridService,CausesDataService){
    var modalPath = PARTIALPATH.modal
    var modalInfoPath = PARTIALPATH.modalInfo

    $scope.grid = {};
    $scope.grid.delete=function(){grdDelete($scope.causeId)};
    $scope.grid.edit=function(){grdEdit($scope.causeId)};
    $scope.grid.refresh=grdRefresh;
    $scope.grid.add=grdAdd;

    $scope.grid.config = JQGridService.config({
    url: PATH.causes + '/getCauses',
    ...
    caption: "Causas",
        onSelectRow: function(id){
            $timeout(function(){
                $scope.selRow = id;
                $scope.causeId = $scope.grid.apicall('getRowData', id).id
            },0);
            
        },
        loadComplete: function (data) {        
            $timeout(function(){
                CausesDataService.setData(data[0]); 
                $scope.selRow =null;
                $scope.causeId = null;
            },0);
            JQGridService.resize('grdCauses');
        }
    },{id:"id"});
    

    function grdEdit(ide){
        console.log(ide);
        if(ide)
           $state.go('causes.edit',{causeId:ide})
        else{
            var modalOptions = {
