function EstatusIndexController($scope,$timeout,$state,$stateParams,PATH,PARTIALPATH,ModalService,JQGridService,EstatusDataService){
    var modalPath = PARTIALPATH.modal
    var modalInfoPath = PARTIALPATH.modalInfo

    $scope.grid = {};
    $scope.grid.delete=function(){grdDelete($scope.estatuId)};
    $scope.grid.edit=function(){grdEdit($scope.estatuId)};
    ...
    {
        JQGridService.resize('grdEstatus');
    };
    $scope.init = function() 
    {
        $scope.loading = true;
    ...
    $scope.grid.config = JQGridService.config({
        url: PATH.estatus + '/getEstatus',
        colNames: ["id", "Estatus"],
        colModel: [
            {name: "id", width: 40, align: "right", hidden: true},
            {name: "estatu", index: "estatu", width: 120},
        ],
        sortname: "name",
        sortorder: "asc",
        ...
        onSelectRow: function(id){
            $timeout(function(){
                $scope.selRow = id;
                $scope.estatuId = $scope.grid.apicall('getRowData', id).id
            },0);
            
        },
        loadComplete: function (data) {        
            $timeout(function(){
                EstatusDataService.setData(data[0]); 
                $scope.selRow =null;
                $scope.estatuId = null;
            },0);
            $scope.loading = false;
    ...
    function grdEdit(id){
        if(id)
           $state.go('estatus.edit',{estatuId:id})
        else{
            var modalOptions = {
                actionButtonText: 'Aceptar',
    ...
    function grdDelete(id){
        if (id) {
            ...
            ModalService.showModal({templateUrl: modalPath}, modalOptions).then(function (result) {
                $scope.loading = true;
                EstatusDataService.delete({id: id})
    ...
    $scope.appFilter = function(filter)
    {
        ...
        $("#grdEstatus").setGridParam({datatype: 'json', postData: { filtro : buscar} , page : 1}).trigger('reloadGrid');
    };
    
    $scope.fncExportar = function() 
    {
        var postData = $('#grdEstatus').getGridParam("postData");
        $scope.nombreArchivo = "estatus " + $scope.fecha.toDateString();
        var data = {
            metodo: 'exportar',
