<?php 
	...
	if ($result->num_rows>0){
		...
		$responseEval['success']=false;
		$response = false;
		echo json_encode($responseEval);
	}
	...
	//Se eliminó el custprov del if para que puedas hacer la validación en la app y se agregó en el array
	//Aquí se valida y se envian los datos de user 3,1,2,4, 1 admin es proveedor hasta donde recuerdo, 3,2,4 son clientes
	//Te estoy enviando custprov en todos los casos en el array con la key "custprov"
	if ($user_type == 1 || $user_type == 2 || $user_type == 4) {
		if ($response == true) {
			...
			while($row = mysqli_fetch_array($result)){
				...
				//Crear array
				//Necesito que me digas que campo quieres cambiar de aquí, tengo entendido que es el "empresa" el que hay que mover segun el caso de cliente o proveedor
	...
	//Usuario Proveedor	
	...
	else if($custprov == 2 && $user_type == 3){
		//ÉSTE ES EL CONDICIONAL DOS PUNTOS UVE
		$sql = "
			SELECT operation_types.*, transport_operations.*, transportoperation_details.* , letter_bearing.folio 
			FROM transportoperation_details
			LEFT JOIN transport_operations 
			ON transportoperation_details.id_operation = transport_operations.id_transport_operation 
			LEFT JOIN operation_types
			ON transport_operations.id_type = operation_types.id_operation_types
			LEFT JOIN letter_bearing
			ON transport_operations.id_note = letter_bearing.id_letter_bearing
			WHERE transport_operations.id_user = '$id_user'
			UNION
			SELECT operation_types.*, transport_operations.*, transportoperation_details.* , letter_bearing.folio 
			FROM transportoperation_details
			RIGHT JOIN transport_operations
			ON transportoperation_details.id_operation = transport_operations.id_transport_operation
			RIGHT JOIN operation_types
			ON transport_operations.id_type = operation_types.id_operation_types
			RIGHT JOIN letter_bearing
			ON transport_operations.id_note = letter_bearing.id_letter_bearing
			WHERE transport_operations.id_user = '$id_user'
			ORDER BY id_transport_operation DESC";
		$result = mysqli_query($conn, $sql) or die (mysqli_error($conn));
		if ($result->num_rows>0) {
		 	while ($row = mysqli_fetch_array($result)) {
		 		$id = $row['id_transport_operation'];
	...
	else if($custprov == 1 && $user_type == 3){
		...
		$sql = "
			...
			LEFT JOIN letter_bearing
			ON transport_operations.id_note = letter_bearing.id_letter_bearing
			WHERE id_company_customer = '$id_company'
			UNION
			SELECT operation_types.*, transport_operations.*, transportoperation_details.* , letter_bearing.folio 
			FROM transportoperation_details
			RIGHT JOIN transport_operations
	...
	//Se valida el usuario tipo 5, el cual les CHOFER, también te envió custprov
	}else if($user_type == 5){
