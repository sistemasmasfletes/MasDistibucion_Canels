...
//		}
    ...
    function ResolveSource()
    {
        ...
        {
            $this->sourceFilename = $this->ResolveFilenameToAbsolute($this->config_error_message_image_default);
            //return $this->ErrorImage('"'.$this->sourceFilename.'" does not exist',50,50);
    ...
    function ImageMagickVersion($returnRAW=false)
    {
        ...
        if (is_null($versionstring))
        {
            ...
            if ($commandline)
            {
                ...
                $this->DebugMessage('ImageMagick convert --version says "'.(isset($matches[0])?$matches[0]:'') .'"', __FILE__, __LINE__);
            }
        }
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
        ...
        if (!$this->gdimg_source)
        {
            ...
            if (!$this->gdimg_source)
            {
                ...
                } elseif (substr($HeaderFourBytes, 0, 3) == "\xFF\xD8\xFF")
                { // JPEG
                ...
                } elseif (substr($HeaderFourBytes, 0, 3) == 'GIF')
                { // GIF
    ...
    function ErrorImage($text, $width=50, $height=50, $forcedisplay=false)
    {
